...
class Portfolio:
    ...
    A Portfolio is also able to optimize itself be checking if the current performance of a trade already
    ...
    by the alpha_model (deviation from the equilibrium of two stocks), which is effectively the expected
    return because we want to capitalize on the fluctuation of difference of stock prices from their equilibrium.
    """

    def __init__(self, account_number, slots, budget, is_test=False):
    ...
    async def initialize(self):
            ...
            for ticker in list(self.portfolio.keys()):
                ...
                try:
                    with sqlite3.connect(PATH / DATABASE_NAME) as log:
                        cur = log.cursor()
                        execution_command = (
                            """SELECT * FROM Signals WHERE Ticker_a = ? OR Ticker_b = ?"""
                        ...
                        if data is None:
                            print(
                                f"\033[32mPORTFOLIO MODEL\033[0m : No Signal to retrieve for {ticker};"
                            )
                    ...
                    print(
                        "\033[32mPORTFOLIO MODEL\033[0m : Signal retrieval failed due to database error;"
                    )
                    print(e)
        else:
            self.followed_signals = {}
        ...
        print(
            f"\033[32mPORTFOLIO MODEL\033[0m : Slot value detected with {initial_slot_value};"
        )
        self.empty_slots = copy.copy(
            initial_slot_value
        )  # Each signal involves two stocks.

    def analyze_signal(self, new_signal):
        """
        Analyzes a new signal and determines if it should be followed.
        Signals are generated by the alpha_model.generate_signals() function.
        :param new_signal: A tuple containing the deviation, sign, pair, quotes, const, slope of the signal.
        """
        ...
        # TODO This is a weak decision-base because Stocks of different tickers can also have a strong correlation.
        if not set(symbols).isdisjoint({ticker_a, ticker_b}):
            self.ignored_signals[pair.export_essentials()[0][0]] = new_signal
            self.ignored_signals[pair.export_essentials()[0][1]] = new_signal
            print(
        ...
        # If slots are vacant, we fill the slot immediately.
        if self.empty_slots > 0:

            # This formula is a result of a linear system of equations that was solved for each quantity of shares.
            ...
            # We need to check if the deviation is positive or negative and adjust the quantity of shares accordingly.
            if sign > 0:
            ...
            # We preemptively block a slot.
            # This will be reversed in case the trade can not be processed successfully.
            self.all_slots -= 1

            # Trigger Portfolio Adjustment Event for further processing.
            EventBus.fire_custom_event("PortfolioAdjustmentEvent", self, portfolio_adjustment)
        else:
            self.ignored_signals[pair.export_essentials()[0][0]] = new_signal
            self.ignored_signals[pair.export_essentials()[0][1]] = new_signal
            print(
                f"\033[32mPORTFOLIO MODEL\033[0m : New Signal will be IGNORED - No slot available; ticker_a = {ticker_a};"
                f" ticker_b = {ticker_b};"
            )

    def optimize(self):
        # First, we gather the current positions directly if any exist.
        if not ib.portfolio():
            print("\033[32mPORTFOLIO MODEL\033[0m : No Portfolio detected;")
            return (
                {}
            )  # This is crucial because the execution_model requires a dict as input.
        else:
        ...
        # We sort the ignored_signals in descending order (the biggest element first) after their "deviation" parameter, the expected return.
        ...
        for symbol in symbols:
            ...
            for signal in current_ignored_signals:
                ...
                print(
                    f"""\033[32mPORTFOLIO MODEL\033[0m : {ticker_a} and {ticker_b} have no alternative signal"""
            ...
            if relative_earnings >= deviation:
                ...
                portfolio_adjustment[ticker_b] = 0
                self.empty_slots += (
                    1  # As the position ought to be clear we can increase this.
                )
                ...
                self.followed_signals[ignored_ticker_a] = self.ignored_signals[
                    ignored_ticker_a
                ]
                self.followed_signals[ignored_ticker_b] = self.ignored_signals[
                    ignored_ticker_a
                ]
        ...
        portfolio_adjustment.update(
            new_positions
        )  # This is not the only occasion portfolio_adjustment receives data.
